...
If ((Get-ChildItem *.pdb -Path .\lib -Recurse).Count -gt 0) {
	$packageTask = Create-Process .\tools\NuGet.exe ("pack Package.nuspec -Symbol -Verbosity Detailed -Prop Configuration=$Configuration")
...
Else {
	$packageTask = Create-Process .\tools\NuGet.exe ("pack Package.nuspec -Verbosity Detailed -Prop Configuration=$Configuration")
	Run-Process-Wait-For-Message -process $packageTask -message "Successfully created package"
	$global:ExitCode = $packageTask.ExitCode
}
